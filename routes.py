...
@app.route('/dashboard')
def dashboard():
    documents = Document.query.order_by(Document.uploaded_at.desc()).limit(50).all()
...
def process_document(document_id):
    ...
    try:
        ...
        # Step 2: Classify document using hybrid classifier
        current_app.logger.info(f"Classifying document {document.filename}")
...
def get_document_stats():
    ...
    try:
        ...
        for doc in docs:
            type_distribution[doc.document_type or 'Unknown'] += 1
...
def analytics_charts():
    try:
        # 1️⃣ Document Type Distribution
...
def analytics_performance():
    try:
        ...
        classification_accuracy = (high_confidence / total_docs) if total_docs > 0 else 0
